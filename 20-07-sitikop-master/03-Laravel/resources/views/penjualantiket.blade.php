...
<body>
    <center><h2>Form Penjualan Tiket </h2></center>
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Penjualan Tiket</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
      <form action="penjualantiket/tambah" method="POST" enctype="multipart/form-data">
                                {{ csrf_field() }}
  ...
  <table class="table table-striped" id="nowplaying">
                <thead>
                    <tr>
            ...
            <div class="modal fade" id="modal_edit_{{$p->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                        <form id="app_form_{{$p->id}}" action="{{ url('nowplaying/update', [$p->id]) }}" method="POST" enctype="multipart/form-data">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}" form="app_form_{{$p->id}}" />
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" clsass="form-control" name="Stock" value="{{$p->Stock}}">
                                </div>
                ...
                <div class="modal fade" id="modal_hapus_{{$p->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                            <a href="{{ url('nowplaying/destroy', [$p->id]) }}" class="btn btn-danger">Save Changes</a>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    var row = 0;
        $(document).ready(function() {
            ...
            $('#penjualantiket thead tr').clone(true).appendTo( '#penjualantiketthead' );
    $('#penjualantiket thead tr:eq(1) th').each( function (i) {
        if (row < 2) {
            var title = $(this).text();
    ...
    var table =   $('#penjualantiket').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "{{ route('ajaxdata.penjualantiket') }}",
